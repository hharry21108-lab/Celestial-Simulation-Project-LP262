...
int main() {
    ...
    std::mt19937 gen(42); // Fixed seed for reproducible "randomness"
    std::normal_distribution<double> distPos(0.0, 0.05 * AU); // 0.05 AU spread
    std::normal_distribution<double> distVel(0.0, 150.0);      // 150 m/s velocity jitter

    // Generate 100 Asteroids for L4 and 100 for L5
    for (int i = 0; i < 100; ++i) {
        // --- L4 Cluster (60 degrees ahead) ---
        double angle4 = M_PI / 3.0;
        glm::dvec3 basePos4(AU * cos(angle4), 0, -AU * sin(angle4));
        glm::dvec3 baseVel4(earth_v * sin(angle4), 0, earth_v * cos(angle4));
        
        glm::dvec3 randPos4(distPos(gen), distPos(gen) * 0.1, distPos(gen)); // Flatter in Y-axis
        glm::dvec3 randVel4(distVel(gen), distVel(gen) * 0.1, distVel(gen));
        
        b.emplace_back("L4_Ast_" + to_string(i), 1e12, basePos4 + randPos4, baseVel4 + randVel4, glm::vec3(0.4f, 0.8f, 0.4f));

        // --- L5 Cluster (60 degrees behind) ---
        double angle5 = -M_PI / 3.0;
        glm::dvec3 basePos5(AU * cos(angle5), 0, -AU * sin(angle5));
        glm::dvec3 baseVel5(earth_v * sin(angle5), 0, earth_v * cos(angle5));
        
        glm::dvec3 randPos5(distPos(gen), distPos(gen) * 0.1, distPos(gen));
        glm::dvec3 randVel5(distVel(gen), distVel(gen) * 0.1, distVel(gen));
        
        b.emplace_back("L5_Ast_" + to_string(i), 1e12, basePos5 + randPos5, baseVel5 + randVel5, glm::vec3(0.4f, 0.4f, 0.8f));
    }
