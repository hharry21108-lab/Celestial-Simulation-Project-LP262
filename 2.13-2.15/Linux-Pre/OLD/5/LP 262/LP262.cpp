...
void compute_acceleration(int id, int n, double& ax, double& ay) {
    ...
    for (int j = 0; j < n; ++j) {
        ...
        // --- 广义相对论 1PN 修正项 ---
        // 我们采用 Einstein-Infeld-Hoffmann (EIH) 近似的简化版本
        // 重点修正：水星进动主导项
        double rel_modifier = 1.0;
        
        if (b[j].mass > 1e29) { // 仅计算中心大质量天体（太阳）产生的相对论效应
            double v2 = b[id].vx * b[id].vx + b[id].vy * b[id].vy;
            double r_dot_v = (dx * b[id].vx + dy * b[id].vy);
            
            // 修正系数：(1 + [修正项]/c^2)
            // 该公式来源于 Schwarzschild 度规下的有效势能修正
            double prec_factor = (4.0 * G * b[j].mass / r - v2) / C2;
            double velocity_term_x = (4.0 * r_dot_v * b[id].vx) / (C2 * r2);
            double velocity_term_y = (4.0 * r_dot_v * b[id].vy) / (C2 * r2);

            ax += f_newton * (dx * (1.0 + prec_factor) + velocity_term_x * r2);
            ay += f_newton * (dy * (1.0 + prec_factor) + velocity_term_y * r2);
        } else {
            ax += f_newton * dx;
            ay += f_newton * dy;
        }
...
void physics_worker_pooled(int thread_id, int num_threads, int n, barrier<>& sync) {
    ...
    for (int step = 0; !quit; ++step) {
        ...
        // 内部小循环：连续计算 20 步才进行一次 UI 同步，彻底消除电流声
        for (int sub = 0; sub < 20; ++sub) {
            for (int stage = 0; stage < 3; ++stage) {
                // 更新位置：当前线程处理自己负责的所有天体
                for (int i = start_idx; i < end_idx; ++i) {
                    b[i].x += b[i].vx * c[stage] * dt;
                    b[i].y += b[i].vy * c[stage] * dt;
                    shared_x[i] = b[i].x;
                    shared_y[i] = b[i].y;
                }
                sync.arrive_and_wait(); // 线程组同步

                // 计算加速度与更新速度
                for (int i = start_idx; i < end_idx; ++i) {
                    double cur_ax, cur_ay;
                    compute_acceleration(i, n, cur_ax, cur_ay);
                    b[i].ax = cur_ax; b[i].ay = cur_ay;
                    b[i].vx += b[i].ax * d[stage] * dt;
                    b[i].vy += b[i].ay * d[stage] * dt;
                }
