...
void render_loop() {
    if (!glfwInit()) return;
    GLFWwindow* window = glfwCreateWindow(1440, 900, "Solar Lab Pro - Camera Swapped", NULL, NULL);
    ...
    while (!glfwWindowShouldClose(window) && !quit) {
        ...
        if (!ImGui::GetIO().WantCaptureMouse) {
            ...
            // 左键：自由模式下的平移 (Swapped)
            if (freeCamera && glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_LEFT) == GLFW_PRESS) {
        ...
        if (showUI) {
            ...
            ImGui::TextColored(ImVec4(1, 1, 0, 1), "Right-Drag: Rotate | Left-Drag: Move (Free)");
