...
// --- 渲染循环 (v3.9 视角锁定与天体循环跟随版) ---
void render_loop() {
    SDL_Init(SDL_INIT_VIDEO); TTF_Init();
    int W = 1400, H = 800;
    auto win = SDL_CreateWindow("Celestial Lab v3.9 - Lagrange Study", 100, 100, W, H, SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE);
    ...
    while (!quit) {
        ...
        while (SDL_PollEvent(&e)) {
            ...
            if (e.type == SDL_KEYDOWN) {
                ...
                case SDLK_MINUS:  replay_speed = max((int)replay_speed - 1, 1); break;
                    ...
                    // --- 新增：视角锁定逻辑 ---
                case SDLK_TAB: // 循环切换跟随天体
                    follow_id++;
                    if (follow_id >= (int)b.size()) follow_id = -1; // 回到自由模式
                    break;
                case SDLK_l: // 快速锁定地球 (假设 ID 为 3) 或解锁
                    if (follow_id == 3) follow_id = -1;
                    else follow_id = 3;
                    break;
                case SDLK_ESCAPE: follow_id = -1; break; // 取消跟随
        ...
        if (follow_id != -1 && follow_id < (int)b.size()) {
            ...
            offY = (H / 2.0) - b[follow_id].y * scale;
        }
        ...
        // 显示当前跟随状态
        if (follow_id != -1) {
            draw_text("FOLLOWING: " + b[follow_id].name + " [TAB/ESC to toggle]", W - 20, H - 40, { 255, 255, 0, 255 }, true);
        }
        ...
        if (show_details) {
            int start_y = 20;       // 起始高度
            int py = start_y;       // 当前绘制高度
            int column_width = 220; // 每一列的宽度
            int col = 0;            // 当前处于第几列（从右往左算）

            for (auto& p : b) {
                // 计算当前列的 X 坐标偏移（从右侧边缘向左偏移）
                int cur_x = W - 20 - (col * column_width);

                // 检查是否即将溢出屏幕底部 (预留 100 像素给操作指南)
                if (py + 80 > H - 100) {
                    py = start_y;   // 回到顶部
                    col++;          // 开启新的一列
                    cur_x = W - 20 - (col * column_width); // 更新 X 坐标
                }
...
int main() {
    ...
    b = {
        ...
        // 哈雷彗星：目前在远日点附近，速度极慢，逆向运行（vy符号根据偏好设置）
        {"Halley",  2.2e14, 35.08 * AU, 0, 0, 910, 0, 0, {255, 255, 255, 255}}
    };
/*
    // 3. 批量生成特洛伊小行星群 (Trojan Swarms)
    std::mt19937 gen(42);
    // 位置扰动幅度：约 0.05 AU (拉格朗日点是一个宽阔的稳定区)
    std::normal_distribution<double> pos_dist(0.0, AU * 0.05);
    // 速度扰动幅度：约 200 m/s
    std::normal_distribution<double> vel_dist(0.0, 200.0);

    // 生成 L4 群 (地球前方 60 度)
    for (int i = 0; i < 100; ++i) {
        double px = -dist_e * cos60 + pos_dist(gen);
        double py = -dist_e * sin60 + pos_dist(gen);
        double vx = v_e * sin60 + vel_dist(gen);
        double vy = -v_e * cos60 + vel_dist(gen);
        b.push_back({ "T4_" + to_string(i), 1e10, px, py, vx, vy, 0, 0, {0, 255, 150, 150} });
    }
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        double vx = -v_e * sin60 + vel_dist(gen);
        double vy = -v_e * cos60 + vel_dist(gen);
        b.push_back({ "T5_" + to_string(i), 1e10, px, py, vx, vy, 0, 0, {255, 150, 255, 150} });
    }*/
