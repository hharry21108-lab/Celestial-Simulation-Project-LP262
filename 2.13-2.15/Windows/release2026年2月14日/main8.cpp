...
void render_loop() {
    ...
    TTF_Font* font = TTF_OpenFont("C:/Windows/Fonts/arial.ttf", 13);// 请确保路径下有此字体，或改为你系统中的字体路径
    ...
    while (!quit) {
        ...
        SDL_SetRenderDrawColor(ren, 10, 10, 25, 255);//绘制背景
        SDL_RenderClear(ren);
        ...
        if (show_grid) {//网格与坐标轴
            ...
            for (double x = ceil(((0 - offX) / scale) / step_world) * step_world; x * scale + offX <= W; x += step_world) {// 垂直线
                int sx = (int)(x * scale + offX);
                SDL_RenderDrawLine(ren, sx, 0, sx, H);
                draw_text(format_unit(x), sx + 5, H - 20, { 120, 120, 150, 255 });
            }            
            for (double y = ceil(((0 - offY) / scale) / step_world) * step_world; y * scale + offY <= H; y += step_world) {// 水平线
