...
const int n = 3;       //天体数量
const int steps = 2e8; //模拟步数
const int MAX_TRAIL_POINTS = 50000; // 增加轨迹点数量
...
void render_thread(vector<Body>& b) {
    ...
    auto draw_legend = [&]() {
        ...
        for (int i = 0; i < static_cast<int>(b.size()); i++) {
            ...
            string bodyName;
            switch (i) {
            case 0: bodyName = "Earth"; break;
            case 1: bodyName = "Moon"; break;
            case 2: bodyName = "Sun"; break;
    ...
    const Uint32 frame_interval = 16; // 约60 FPS (1000/60 ≈ 16.67ms)

    while (!quit) {
        // 先处理所有SDL事件，确保暂停后能正常唤醒
...
int main() {
    ...
    // ==================== 完美的地日月初始条件 ====================
    // 物理常数
    const double m_s = 1.989e30;           // 太阳质量 (kg)
    const double m_e = 5.972e24;           // 地球质量 (kg)
    const double m_m = 7.348e22;           // 月球质量 (kg)
    const double d_earth_sun = AU;         // 地球到太阳距离 (m)
    const double d_earth_moon = 3.844e8;   // 地球到月球距离 (m)

    // 计算轨道速度
    const double v_earth_orbit = sqrt(G * m_s / d_earth_sun);      // 地球绕太阳的轨道速度: 29780 m/s
    const double v_moon_orbit = sqrt(G * m_e / d_earth_moon);      // 月球绕地球的轨道速度: 1018 m/s

    // 计算总动量
    const double p_earth = m_e * v_earth_orbit;                    // 地球动量: 1.778e29 kg·m/s
    const double p_moon = m_m * (v_earth_orbit + v_moon_orbit);    // 月球动量: 2.271e27 kg·m/s
    const double p_total = p_earth + p_moon;                       // 总动量: 1.8007e29 kg·m/s

    // 太阳的动量必须与地球和月球的总动量相反
    const double p_sun = -p_total;                                 // 太阳动量: -1.8007e29 kg·m/s
    const double v_sun = p_sun / m_s;                              // 太阳速度: -0.0905 m/s

    // ==================== 天体初始状态 ====================
    // 1. 太阳 (中心)
    b[2].mass = m_s;                    // 1.989e30 kg
    b[2].x = 0.0;                       // 0 m
    b[2].y = 0.0;                       // 0 m
    b[2].vx = 0.0;                      // 0 m/s
    b[2].vy = v_sun;                    // -0.0905 m/s

    // 2. 地球 (绕太阳公转)
    b[0].mass = m_e;                    // 5.972e24 kg
    b[0].x = -d_earth_sun;              // -1.496e11 m
    b[0].y = 0.0;                       // 0 m
    b[0].vx = 0.0;                      // 0 m/s
    b[0].vy = v_earth_orbit;            // 29780 m/s

    // 3. 月球 (绕地球公转)
    b[1].mass = m_m;                    // 7.348e22 kg
    b[1].x = -d_earth_sun + d_earth_moon; // -1.492156e11 m
    b[1].y = 0.0;                       // 0 m
    b[1].vx = 0.0;                      // 0 m/s
    b[1].vy = v_earth_orbit + v_moon_orbit; // 30798 m/s
    // 双缓冲：上一轮坐标快照
