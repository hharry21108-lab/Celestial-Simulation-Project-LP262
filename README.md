...
$$
\vec{F}_{\text{sun}} = G\frac{M_{\text{sun}} m}{r_{\text{tosun}}^2}
$$

地球引力

$$
\vec{F}_{\text{earth}} = G\frac{M_{\text{earth}} m}{r_{\text{toearth}}^2}
$$
...
$$
F_i = \sum_{\substack{j=1 \\ j \ne i}}^{n} G \frac{m_i m_j}{\lvert \vec{r}_j - \vec{r}_i \rvert^2}
$$
...
$$
a_{x,i} = \sum_{\substack{j=1 \\ j \ne i}}^{n} G m_j \frac{x_j - x_i}{\lvert \vec{r}_j - \vec{r}_i \rvert^3}
$$

$$
a_{y,i} = \sum_{\substack{j=1 \\ j \ne i}}^{n} G m_j \frac{y_j - y_i}{\lvert \vec{r}_j - \vec{r}_i \rvert^3}
$$
...
```cpp
		auto worker = [&](int id) {
		    for (int step = 0; step < steps; ++step) {
		        ...
		        // 计算全步速度
		        double ax2, ay2;
		        compute_acceleration(id, n, ax2, ay2); // 计算第二次加速度
		        b[id].vx += 0.5 * ax2 * dt;
		        b[id].vy += 0.5 * ay2 * dt;
		        old_x[id] = b[id].x;
		        old_y[id] = b[id].y;
...
- **最终推演，验证理论（带入之前坐标）**
		...
		![L4/L5稳定性模拟](天体模拟LP262-1774886221762.webp)
		![系统模拟结果](天体模拟LP262-1774886308451.webp)
4. **结果**
